...
TURN_CORRECTION_RATIO = 0.995
FORWARD_CORRECTION_RATIO = 0.005
# GLOBAL VARIABLE

class iRobot_create3:
    ...
    async def turn_left(self, angle=90):
        print(f"iRobot: turn {angle} degree left, actual degree turned {angle * TURN_CORRECTION_RATIO}")
        self._change_orientation(-angle) #We define ccw to be negative
        await self.irobot.turn_left(angle * TURN_CORRECTION_RATIO)
    
    async def turn_right(self, angle=90):
        print(f"iRobot: turn {angle} degree right, actual degree turned {angle * TURN_CORRECTION_RATIO}")
        self._change_orientation(angle) #We define cw to be positive
        await self.irobot.turn_right(angle * TURN_CORRECTION_RATIO)
    ...
    async def forward_by_distance(self, distance=BLOCK_UNIT):
        ...
        #await self.irobot.turn_left(90)
        #await self.irobot.move(2 + distance * FORWARD_CORRECTION_RATIO)
        #await self.irobot.move(-2)
        #await self.irobot.turn_right(90 - distance * FORWARD_CORRECTION_RATIO * 0.5)
    ...
    def _forward(self, distance):
        x_displacement = distance * np.cos(self.orientation[-1] / 180 * np.pi)
        y_displacement = distance * np.sin(self.orientation[-1] / 180 * np.pi)
    ...
    def get_extrinsic_matrix(self):
        return BEV.get_extrinsic_matrix(0, math.radians(self.orientation[-1]), 0, self.position[-1][1], 0, self.position[-1][0])
    
    def get_extrinsic_matrix_o3d(self):
        #x_displacement = np.sin(-math.radians(self.orientation[-1])) * 7 + np.cos(-math.radians(self.orientation[-1])) * 4
        #y_displacement = np.cos(-math.radians(self.orientation[-1])) * 7 + np.sin(-math.radians(self.orientation[-1])) * 4
        x_displacement = 0
        y_displacement = 0
        return BEV.get_extrinsic_matrix(0, -math.radians(self.orientation[-1]), 0, (self.position[-1][1] + x_displacement) / 1100, 30, (self.position[-1][0] + y_displacement) / 1100)
    ...
    async def play(robot):
        while True:
            ...
            if userinput == "l" or userinput == "left":
                ...
                angle = parse_integers_from_string(userinput)[0]
                ...
                distance = parse_integers_from_string(userinput)[0]
